...
```{r}
anno <- t.test(
  tree_transect_data_full[tree_transect_data_full$class == "seedling" & tree_transect_data_full$country == "Malla", "n"],
  tree_transect_data_full[tree_transect_data_full$class == "seedling" & tree_transect_data_full$country == "Storfjord", "n"]
)$p.value
```
...
```{r}
vegetation_data %>% 
  ggplot(aes(x = type, y = cover, fill = country)) +
  geom_boxplot()
...
x <- read.csv("../../2_input/1_raw_data/reindeer_numbers.csv") %>% 
  select(Year, Bassevuovdi, Kasivarsi) %>% 
...
ggplot(x, aes(x = Year, y = n_KM2, color = District)) +
  ...
  scale_color_manual(values = c("#77BFBC", "#EB956A"), labels = c("Bassevuovdi (winter)", "Käsivarsi (year-round)")) +
  theme_classic() + 
  theme(legend.position = c(0.8, 0.5),
        panel.border = element_rect(color = "black", fill = NA, size = 1))

ggsave("../../3_output/2_graphs/publication_ready/reindeer_numbers.svg",
       height = 4, width = 6)
```
...
data$country <- factor(data$country, levels = c("Malla", "Storfjord"), labels = c("Year-round", "Winter"))
...
p1 <- data %>%  
  ...
  facet_grid(~ factor(country, levels=c( "Winter", "Year-round"))) +
  scale_color_manual(values = c("#EB956A", "#77BFBC"), labels = c("Year-round", "Winter")) + 
  scale_fill_manual(values = c("#EB956A", "#77BFBC"), labels = c("Year-round", "Winter")) +
  theme(panel.spacing = unit(.0, "lines"),
        panel.border = element_rect(color = "black", fill = NA, size = 1), 
...
ggsave("../../3_output/2_graphs/publication_ready/leaf_sections.svg", height = 4, width = 5)
```
...
ggsave("../../3_output/2_graphs/publication_ready/root_shoots.svg", height = 4, width = 4)
```
...
ggsave("../../3_output/2_graphs/publication_ready/tree_morphology.svg", height = 4, width = 7)
```

...
p1 <- dung_transect_data %>%
  ...
  scale_x_discrete(labels=c("forest" = "forest", "treeline" = "low tundra",
                              "tundra" = "high tundra")) +
  theme(panel.spacing = unit(.0, "lines"),
        panel.border = element_rect(color = "black", fill = NA, size = 1), 
        strip.background = element_rect(color = "black", size = 1),
        axis.text.x = element_text(angle = 15, hjust = 1),
        legend.position =  "top", legend.margin = margin(t = -4)); print(p1)
...
#  geom_jitter(aes(), alpha = 0.3, position = #position_jitterdodge(jitter.width = 0, dodge.width = 0.75)) + 
  ...
  theme(panel.spacing = unit(.0, "lines"),
        ...
        strip.background = element_rect(color = "black", size = 1),
        axis.text.x = element_text(angle = 25, hjust = 1)); print(p1)
...
```{r echo = FALSE}
ef <- y %>% 
  select(NH4, CN_ratio, country, habitat) %>% 
  envfit(sel_cca, ., na.rm = TRUE); ef
```

```{r}
ef_coord_cont <-  as.data.frame(scores(ef, "vectors")) * ordiArrowMul(ef) # map vectors in plot
#ef_coord_cont <- ef_coord_cont[!(row.names(ef_coord_cont) %in% c("slope", "wN", "wC")),]
rownames(ef_coord_cont) <- c("ammonium", "C/N ratio") 
ef_coord_cont <- ef_coord_cont * 2

ef_coord_cat = as.data.frame(scores(ef, "factors")) * ordiArrowMul(ef) # map factors in plot
ef_coord_cat <- ef_coord_cat[1:6,]
rownames(ef_coord_cat) <- c("Finland", "Norway", "forest", "treeline", "tundra", "high tundra")
ef_coord_cat <- ef_coord_cat * 1.5
ef_coord_cat <- ef_coord_cat[3:6,]

# get CCA scores ready for plotting
species_cca <- fortify(sel_cca) %>% 
  filter(score == "species")
species_cca[, 3:8] <- species_cca[, 3:8] * 3
species_cca$label <- c("Salix", "B. nana", "decid. dwarf shrubs", "J. communis", "evergr. dwarf shrubs", "graminoids", "forbs", "ferns", "bryophytes", "lichens", "bare ground", "rock")
...
p2 <- ggplot() + 
  ...
  scale_fill_manual(values = c("#EB956A", "#77BFBC"), labels = c("Year-round", "Winter")) +
  scale_shape_manual(values = c(15, 16, 17, 3), labels = c("Forest", "Forest edge", "Low tundra", "High tundra")) + 
  geom_text(data = species_cca,aes(x = CCA1, y = CCA2, label = label), size = 4,  alpha = 0.7) +  # add the species labels
  #geom_text(data = data_scores, aes(x = NMDS1, y = NMDS2, label = plot), vjust=0) +  # add the site labels
  ...
  guides(color = guide_legend(override.aes = list(shape = NA))) +
  theme(panel.border = element_rect(color = "black", fill = NA, size = 1), 
        strip.background = element_rect(color = "black", size = 1)); print(p2)
```
...
ggsave("../../3_output/2_graphs/publication_ready/Fig4_CCAs.svg", height = 4, width = 7.5)
```

```{r}
ggsave("../../3_output/2_graphs/publication_ready/FigSx_CCA_select.svg", height = 3.5, width = 6)
```
...
p3 <- cn_soil %>% 
  ...
  scale_fill_manual(values = c("#77BFBC", "#EB956A", "black", "black"), labels = c("Winter", "Year-round")) +
  labs(y = "ωN") +
  theme_classic() +
  theme(axis.text.x=element_blank(), 
        axis.title.x = element_blank(),
...
vegetation_data_l %>% 
  ...
  scale_fill_manual(values = c("#77BFBC", "#EB956A"), labels = c("Low", "High")) +
  scale_x_discrete(labels=c("forest" = "forest", "treeline" = "forest edge", "tundra" = "low tundra", "upper_tundra" = "high tundra")) +
